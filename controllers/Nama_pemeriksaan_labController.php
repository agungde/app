...
class Nama_pemeriksaan_labController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				nama_pemeriksaan_lab.id_jenis LIKE ? OR 
				nama_pemeriksaan_lab.tanggal LIKE ? OR 
				nama_pemeriksaan_lab.nama_pemeriksaan LIKE ? OR 
				nama_pemeriksaan_lab.nilai_rujukan LIKE ? OR 
				nama_pemeriksaan_lab.user_entry LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("nama_pemeriksaan_lab.id_jenis", ORDER_TYPE);
		}
		if($fieldname){
	...
	function import_data(){
		...
		$this->redirect("nama_pemeriksaan_lab");
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("nama_pemeriksaan_lab.id_jenis", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("tanggal","nama_pemeriksaan","nilai_rujukan","user_entry");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id_jenis","tanggal","nama_pemeriksaan","nilai_rujukan","user_entry");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'tanggal' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("nama_pemeriksaan_lab.id_jenis", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("nama_pemeriksaan_lab.id_jenis", $arr_rec_id, "in");

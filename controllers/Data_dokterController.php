...
if($cekdata=="1" or $cekdata=="4"){
	...
	# End of before list statement
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				data_dokter.date_update LIKE ? OR 
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("username","nama_dokter","alamat","jenis_kelamin","no_hp","specialist","email","jasa_poli","jasa_kunjungan","photo","operator","date_update","date_updated");
...
mysqli_query($koneksi,"INSERT INTO `user_login` (`nama`, `email`, `password`, `id_dokter`, `user_role_id`) VALUES ('".$_POST['username']."', '".$_POST['email']."', '$password_hash', '$rec_id', '3')"); 
...
while ($row=mysqli_fetch_array($sqlcek1)){
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id_dokter","nama_dokter","alamat","jenis_kelamin","no_hp","specialist","email","jasa_poli","jasa_kunjungan","photo","operator","date_update","date_updated");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'nama_dokter' => 'required',
	...
	function delete($rec_id = null){
		...
		$db->where("data_dokter.id_dokter", $arr_rec_id, "in");

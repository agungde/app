...
class Data_obatController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				data_obat.id_obat LIKE ? OR 
				data_obat.nama_obat LIKE ? OR 
				data_obat.jumlah LIKE ? OR 
				data_obat.satuan LIKE ? OR 
				data_obat.harga_beli LIKE ? OR 
				data_obat.harga_jual LIKE ? OR 
				data_obat.date_created LIKE ? OR 
				data_obat.date_update LIKE ? OR 
				data_obat.user_entry LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("data_obat.id_obat", ORDER_TYPE);
		}
		if($fieldname){
		...
		$view_name = (is_ajax() ? "data_obat/ajax-list.php" : "data_obat/list.php");
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("data_obat.id_obat", $rec_id);; //select record based on primary key
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("nama_obat","jumlah","satuan","harga_beli","harga_jual","user_entry");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id_obat","nama_obat","jumlah","satuan","harga_beli","harga_jual","user_entry");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'nama_obat' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("data_obat.id_obat", $rec_id);;
	...
	function delete($rec_id = null){
		...
		$db->where("data_obat.id_obat", $arr_rec_id, "in");
